...
@Data
// 페이징 처리 로직을 일일이 변수 선언하여 사용하면 효율성이 떨어지므로 재사용 가능한 객체로 정의한다.
public class PagingUtil {
	int totalRecord; // 총 레코드 수
	int pageSize=10; // 한 페이지당 보여질 레코드 수(참고로 이 변수명은 오라클의 SQLPlus 접속기에서 사용하는 변수명임)
	...
	public void init(List list, HttpServletRequest request) {
		...
		// 사용자가 선택한 페이지 수 처리
		if(request.getParameter("currentPage")!=null) { // 파라미터가 있을때만 사용자가 넘겨준 페이지명을 this.currentPage에 대입하자.
		...
		this.lastPage = this.firstPage + (this.blockSize-1); // 언제나 첫페이지와 blockSize-1만큼의 간격을 둠
		this.curPos=(this.currentPage-1)*this.pageSize;
		this.num=this.totalRecord-(this.currentPage-1)*this.pageSize;
	}
